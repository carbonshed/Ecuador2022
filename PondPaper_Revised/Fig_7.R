...
df_CO2_full <- read.csv(here::here("data/Pond_pCO2_data.csv"))%>%
  filter(Site=="Wetland")%>%
...
drone_co2 <- read.csv(here::here("data/Wetland_edges_df.csv"))%>%
  dplyr::select(pCO2_w_uatm)%>%rename(partialpressure=pCO2_w_uatm)
drone_co2$gas <- "pCO2"

drone_ch4 <- read.csv(here::here("data/Wetland_edges_df.csv"))%>%
...
###plot
p <- ggplot(df_all,aes(x=gas, y=partialpressure,fill=Location)) +
