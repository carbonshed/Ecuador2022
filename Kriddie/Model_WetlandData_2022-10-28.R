...
M2B <- lmer(log1p(Flux_umol_m2_s) ~ CO2_ppm_scale + (1|Wetland),data=df)

M2B <- lmer(log1p(Flux_umol_m2_s) ~ log(CO2_ppm) + (1|Wetland),data=df)
summary(M2B)

#so I think we don't need to use the mixed model because the effect of concentration on flux is independent from wetland. right?
# wait maybe not.
