...
##drivers of CO2 flux
  #all factor that were important to both [CO2] and k
    #Air Temp
    #Water Temp
    #Difference water/air temp
    #depth vol ratio (more contact with sediments)
    #surface area
    #watershed size 
    #precipitation 
    #Solar radiation
    #wind (more turbulance)

#Big model
df$precip_mm_ave2 <- (df$PrecipAccu_mm_PreviousDay + df$PrecipAccuDay_mm)/2
#Big model
M1 <- lmer(log(Flux_umol_m2_s)  ~ scale(AirTemp_c) +
...
#paired down 1
M2 <- lmer(log(Flux_umol_m2_s)  ~ #scale(AirTemp_c) +
          #   scale(BaroTemp_c_yearly)+ 
             scale(waterTemp_c_day) + 
           #  scale(waterTemp_c_yearly) +
            # scale(Water_minus_air_Temp) +
...
#paired down 2
M3 <- lmer(log(Flux_umol_m2_s)  ~ scale(AirTemp_c) +
                scale(BaroTemp_c_yearly)+ 
             scale(waterTemp_c_day) + 
            #   scale(waterTemp_c_yearly) +
            #  scale(Water_minus_air_Temp) +
