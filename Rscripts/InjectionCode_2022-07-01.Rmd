...
#viasalas taken up before return to baseline at x25 and x50
...
#not working until 11:03 (after in water)
...
time: 12:04
salt pour distance: x = -15m
salt added: 238g/l * 1.5L = 357g

x0m
Q = 92.12 L/s
Peak = 7/1/2022  12:05:45 PM

x50m 
Q = 86.56
peak = 7/1/2022  12:13:57 PM (peak time different from shiney app)
-------------
time: 12:47
salt pour distance: x = -15m
salt added: 238g/l * 1.5L = 357g
...
C_mgperL_25m <- (platmean25m - mean(preplatmean25m))
...
#Q at 0m = 0.07623
Q_0m = mean(92.12,91.13) #0.07623 * 1000
# Q for 50m = 0.090885
Q_50m = mean(86.56,88.7) #0.090885 * 1000 
...
#time for slug to go between 0m and 50m 12:05:45 PM	 to	12:13:57 PM
#start: 12:04
# velocity in m/s
dist_salt = 50
diff_one <- (13-5)*60 + (45-57)
velocity_one <- dist_salt / diff_one


#Slug Two

#don't have reliagble second 0m at the moment, so we just use the first elocity for notw

#discharge at 50m; poured in salt at 12:47, peak at 12:57 
#start: 12:47
# velocity in m/s
#diff_two <- (57 - 47) * 60
#velocity_two<- dist_salt / diff_two

#avg_v <- mean(velocity_one, velocity_two)
...
df_avg_water <- filter(df_corr, DateTime >= as.POSIXct("2022-07-01 12:00:00", tz = "UTC") & DateTime < as.POSIXct("2022-07-01 13:00:00", tz = "UTC"))
...
```{r k600 at -10m}
# first, read in WL hobo at -10m 
flux_n10 <- read.csv(here::here("Injections/2022-07-01/Flux_2022-07-01_12-52-51.csv"))
...
flux_n10 <- flux_n10%>%filter(Date == "2022-07-01")
...
backgrond_df <- filter(backgrond_df, DateTime >= as.POSIXct("2022-07-01 11:20:00", tz = "UTC") & DateTime < as.POSIXct("2022-07-01 12:58:00", tz = "UTC"))
#add baro data because we need air temp

baro_n10 <- read.csv(here::here("Injections/2022-07-01/Injection_Baro_xn10_2022-07-01.csv"),skip = 1)[2:4]
...
##calculate k600
########
adjustment <-filter(baro_n10, DateTime >= as.POSIXct("2022-07-01 11:20:00", tz = "UTC") & DateTime < as.POSIXct("2022-07-01 12:58:00", tz = "UTC"))
