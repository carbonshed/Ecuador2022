...
This file is processing CO2 injection data and calculating k600 
...
end <-  as.POSIXct("2022-07-15 17:00:00", tz = "UTC")
...
waterlevel_n10 <- read.csv("Temp_injection_2022-07-15.csv", skip = 1)
...
#30m vaisala data
```{r check CO2 data plot 30m, echo=FALSE}
##check data
plot_ly(df_x30m,
    x = ~DateTime, y = ~adjusted_ppm, type = 'scatter', mode = 'markers') 
```
...
preplat_n10 <- filter(df_xn10m, DateTime >= as.POSIXct("2022-07-15 10:50:00", tz = "UTC") & DateTime < as.POSIXct("2022-07-15 10:55:00", tz = "UTC"))
preplat_0 <- filter(df_x0m, DateTime >= as.POSIXct("2022-07-15 10:45:00", tz = "UTC") & DateTime < as.POSIXct("2022-07-15 10:50:00", tz = "UTC"))
preplat_15 <- filter(df_x15m, DateTime >= as.POSIXct("2022-07-15 10:50:00", tz = "UTC") & DateTime < as.POSIXct("2022-07-15 10:55:00", tz = "UTC"))
preplat_30 <- filter(df_x30m, DateTime >= as.POSIXct("2022-07-15 10:45:00", tz = "UTC") & DateTime < as.POSIXct("2022-07-15 10:50:00", tz = "UTC"))
...
plat_n10 <- filter(df_xn10m, DateTime >= as.POSIXct("2022-07-15 12:20:00", tz = "UTC") & DateTime < as.POSIXct("2022-07-15 12:25:00", tz = "UTC"))
plat_0 <- filter(df_x0m, DateTime >= as.POSIXct("2022-07-15 12:20:00", tz = "UTC") & DateTime < as.POSIXct("2022-07-15 12:25:00", tz = "UTC"))
plat_15 <- filter(df_x15m, DateTime >= as.POSIXct("2022-07-15 12:20:00", tz = "UTC") & DateTime < as.POSIXct("2022-07-15 12:25:00", tz = "UTC"))
plat_30 <- filter(df_x30m, DateTime >= as.POSIXct("2022-07-15 12:20:00", tz = "UTC") & DateTime < as.POSIXct("2022-07-15 12:25:00", tz = "UTC"))
...
#postplat_n10 <- filter(df_xn10m, DateTime >= as.POSIXct("2022-07-15 13:25:00", tz = "UTC") & DateTime < as.POSIXct("2022-07-15 13:30:00", tz = "UTC"))
postplat_0 <- filter(df_x0m, DateTime >= as.POSIXct("2022-07-15 13:25:00", tz = "UTC") & DateTime < as.POSIXct("2022-07-15 13:30:00", tz = "UTC"))
postplat_15 <- filter(df_x15m, DateTime >= as.POSIXct("2022-07-15 13:25:00", tz = "UTC") & DateTime < as.POSIXct("2022-07-15 13:30:00", tz = "UTC"))
postplat_30 <- filter(df_x30m, DateTime >= as.POSIXct("2022-07-15 13:25:00", tz = "UTC") & DateTime < as.POSIXct("2022-07-15 13:30:00", tz = "UTC"))
...
air_n10 <-filter(df_xn10m, DateTime >= as.POSIXct("2022-07-15 09:30:00", tz = "UTC") & DateTime < as.POSIXct("2022-07-15 09:35:00", tz = "UTC"))
air_0 <- filter(df_x0m, DateTime >= as.POSIXct("2022-07-15 09:30:00", tz = "UTC") & DateTime < as.POSIXct("2022-07-15 09:35:00", tz = "UTC"))
air_15 <- filter(df_x15m, DateTime >= as.POSIXct("2022-07-15 09:30:00", tz = "UTC") & DateTime < as.POSIXct("2022-07-15 09:35:00", tz = "UTC"))
air_30 <- filter(df_x30m, DateTime >= as.POSIXct("2022-07-15 09:30:00", tz = "UTC") & DateTime < as.POSIXct("2022-07-15 09:35:00", tz = "UTC"))
...
write.csv(co2_table,here::here("Injections/2022-07-15/injection_co2_result.csv"))
...
adjusted for decay rate of CO2 over meters
 ...
 x0m
Q L/s= 3.32
 peak at 7/15/2022  11:36:23 AM
 
 x15m
 Q L/s = 3.13
 peak at 7/15/2022  11:38:38 AM
 
 x30m
 Q L/s = 6.01 
 peak at 7/15/2022  11:41:03 AM
 ...
 x0m
 Q L/s = 2.9
 7/15/2022  12:00:12 PM
  

 x15m
 Q L/s = 3.26
 peak at 7/15/2022  12:02:27 PM
 
 
 x30m
Q L/s = 5.88
 peak at 7/15/2022  12:04:55 PM
...
C_mgperL_30m <- (platmean30m - mean(c(preplatmean30m,postplatmean30m)))
...
# Slug One @ 11:33
#at 0m  peak at 7/15/2022  11:36:23 AM
#at 30m  peak at 7/15/2022  11:41:03 AM 
# velocity in m/s

dist_salt = 30
diff_one <- (41-36)*60 + (03-23)
velocity_one <- dist_salt / diff_one

# Slug One @ 11:59
#at 0m  peak 7/15/2022  12:00:12 PM
#at 30m   peak at 7/15/2022  12:04:55 PM
# velocity in m/s
dist_salt = 30
diff_two <- (04-00)*60 + (55-12)
...
df_avg_water <- filter(df_corr, DateTime >= as.POSIXct("2022-07-15 09:45:00", tz = "UTC") & DateTime < as.POSIXct("2022-07-15 12:30:00", tz = "UTC"))
...
```{r k600 at -10m}
# first, read in WL hobo at -10m 
flux_n10 <- read.csv(here::here("Injections/2022-07-15/Injection_Flux_xn10m_2022-07-15.csv"))
...
flux_n10 <- flux_n10%>%filter(Date == "2022-07-15")
...
backgrond_df <- filter(backgrond_df, DateTime >= as.POSIXct("2022-07-15 10:30:00", tz = "UTC") & DateTime < as.POSIXct("2022-07-15 12:30:00", tz = "UTC"))
#add baro data because we need air temp

baro_n10 <- read.csv(here::here("Injections/2022-07-15/Injection_Baro_xn10m_2022-07-15.csv"),skip = 1)[2:4]
...
##calculate k600
########
adjustment <-filter(baro_n10, DateTime >= as.POSIXct("2022-07-15 10:30:00", tz = "UTC") & DateTime < as.POSIXct("2022-07-15 12:50:00", tz = "UTC"))
adjustment <- adjustment%>%drop_na(AirPress_kpa)
