...
C_mgperL_15m <- (platmean15m - mean(preplatmean15m,postplatmean15m))
...
avg_v <- mean(velocity_one,velocity_two) # why is mean() not working???
avg_v <- (velocity_one+velocity_two)/2
...
CO2_air_ppm <- airmean_n10m
R=0.08205736608096
gCO2asC <- 12
#convert hpa to atm; 1hPa = 0.0009869233 atm
backgrond_df$air_pressure_atm <- backgrond_df$Total_hPa * 0.0009869233
backgrond_df$VaporPressure_atm <- 10^(5.40221-(1838.675/((backgrond_df$AirTemp_c + 273.15)-31.737)))

backgrond_df$TotalAir_atm <- backgrond_df$air_pressure_atm - backgrond_df$VaporPressure_atm
...
backgrond_df$KH_mol.L.atm <- kH_STP_mol.L.atm * exp(D_K*(1/df$WaterTemp_K - 1/T_STP_K))
...
backgrond_df$deltaCO2_gCO2asCperM3 <-  (backgrond_df$CO2_water_gCO2asCPerLiter - backgrond_df$CO2_air_gCO2asCPerLiter)  *
  backgrond_df$KH_mol.L.atm  * LiterToCubicMeter
...
backgrond_df$Flux_gCO2asCperM2perDay <- backgrond_df$Flux_ave * SecToDay /  UmoleToMole *gCO2asC
...
#T = temperature in c
backgrond_df$Sc <- 1911.1 - 118.11*backgrond_df$WaterTemp_c + 3.4527*(backgrond_df$WaterTemp_c)^2 - 0.04132*(backgrond_df$WaterTemp_c)^3

backgrond_df$K600.effective <- backgrond_df$k_m.d * (600/backgrond_df$Sc)^(-0.5)
