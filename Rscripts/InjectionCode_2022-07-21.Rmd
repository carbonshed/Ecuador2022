...
C_mgperL_15m <- (platmean15m - mean(c(preplatmean15m,postplatmean15m)))

C_mgperL_30m <- (platmean30m - mean(preplatmean30m))

#Discharge comes from hand held
#multiply by 1000 to convert from m3/s to L/s
#Q at 0m = 
Q_0m = mean(c(3.96,3.58))
# Q for 50m = 
Q_50m = mean(c(9.82,9.99))

#stream reach length
dist = 30

km <- -log(C_mgperL_30m*Q_50m/(C_mgperL_0m*Q_0m))/dist
#km units: 1/m
```
...
# 10:45:52  to 10:49:09 AM
dist_salt = 30
diff_one <- (49-45)*60 + (09-52)
velocity_one <- dist_salt / diff_one

#11:13:56 AM to  11:17:36 AM
dist_salt = 30
diff_two <- (17-13)*60 + (36-56)
velocity_two <- dist_salt / diff_two

avg_v <- mean(c(velocity_one,velocity_two)) 
...
df_avg_water <- filter(df_corr, DateTime >= as.POSIXct("2022-07-21 10:16:00", tz = "UTC") & DateTime < as.POSIXct("2022-07-21 11:20:00", tz = "UTC"))
...
```{r k600 at -10m}
# first, read in WL hobo at -10m 
#flux_n10 <- read.csv("Injections_Flux_xn10m_2022_07_21.csv", skip = 1)
...
flux_n10 <- flux_n10%>%filter(Date == "2022-07-21")
...
backgrond_df <- filter(backgrond_df, DateTime >= as.POSIXct("2022-07-21 09:45:00", tz = "UTC") & DateTime < as.POSIXct("2022-07-21 11:15:00", tz = "UTC"))

#add baro data because we need air temp
#missing baro file, so use the installed baro

baro_n10 <- read.csv(here::here("Water_Level/BaroPress/Baro_01_2022-07-27.csv"),skip = 10)[,c(1:2,4:5)]
colnames(baro_n10) <- c("Date","Time","AirPress_kpa","AirTemp_c")
baro_n10$DateTime <- as.POSIXct(paste(baro_n10$Date,baro_n10$Time), format="%m/%d/%Y %I:%M:%S %p", tz = "UTC")
baro_n10 <- baro_n10[,c("DateTime","AirPress_kpa","AirTemp_c")]

#backgrond_df <- left_join(backgrond_df,baro_n10,by="DateTime")

# and now we have to interpolate to 5 min >:(

#interpolate 1hr to 15 min 

DateTime <- seq(ISOdate(2022,07,20), ISOdate(2022,07,21), by = "5 min")
DateTime_df <- data.frame(DateTime)
DateTime_df$DateTime <- as.POSIXct(DateTime_df$DateTime, format="%y-%m-%d %H:%M:%S", tz="UTC")

baro_n10 <- left_join(DateTime_df, baro_n10, by = "DateTime")
baro_n10 <- baro_n10[ order(baro_n10$DateTime , decreasing = TRUE ),]

## Round timestamps to closest 5-minute
timestep <- baro_n10$DateTime[2]-baro_n10$DateTime[1]
baro_n10 <- data.frame(DateTime=seq.POSIXt(baro_n10$DateTime[1], baro_n10$DateTime[length(baro_n10$DateTime)], by=timestep))%>%
...
AirPress_hpa <- mean(backgrond_df$AirPress_kpa)*10
AirTemp_mean <- mean(backgrond_df$AirTemp_c)
