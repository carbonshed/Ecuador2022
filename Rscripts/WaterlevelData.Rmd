...
WL_Wetland02 <- WL_Wetland02%>%filter(DateTime != as.POSIXct("2023-06-20 11:45:00",tz = "UTC")&
  ...
  #string broke, bad data for water level. but lets keep temp
...
ggplot(data = WL_Wetland10, aes(x = DateTime, y = WLPres_kpa)) + geom_point()
...
#export data (dani)
#write.csv(WL_Wetland, "../daniela/WL_data.csv")
#write.csv(WL_Wetland, here::here("Kriddie/WL_data.csv"))
...
#enter wetland dataframe
wetland <- "WL_Wetland04" # example: WL_Wetland01, WL_Wetland02
#code below extract desired wetland, do not change
wetland_data <- WL_Wetland%>%filter(Station==wetland)
#plot_ly(data = wetland_data, x = ~DateTime, y = ~WaterLevel_m)

p3 <- ggplot(wetland_data, aes(x=DateTime,y=depth_ave_m)) +
  geom_point() + 
  labs(x=NULL, y="Depth_ave_m") +
  theme(plot.title = element_text(size = 9)) + 
  geom_hline(yintercept=depth_DF$depth_ave_m[depth_DF$Station==wetland], color="red") +
#  geom_hline(yintercept=Q_low, color="red") +
  theme(text=element_text(size=18))
p3

p4 <- ggplot(data = WL_Wetland, aes(x = DateTime, y = depth_ave_m)) + geom_point()
p4 + facet_wrap(~Station)
...
Sdf2$DateTime <- lubridate::ceiling_date(Sdf2$DateTime, "30 minutes")
...
Sdf3$DateTime <- lubridate::floor_date(Sdf3$DateTime, "30 minutes")
...
Sdf4$DateTime <- lubridate::floor_date(Sdf4$DateTime, "1 hour")
...
ggplot(data = SurfaceArea_df%>%filter(Wetland == "Wetland_3"), aes(x = WL, y = Area, color=Date)) + 
   geom_point(size=3)
...
p2 <- ggplot(data = SurfaceArea_df, aes(x = WL, y = Area)) + geom_point()
