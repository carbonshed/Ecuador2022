...
p.ch4 <- ggplot(df,aes(x=reorder(New.Name,Elevation_m,median,decreasing = TRUE),y=CH4_umol.L)) +
   geom_point(size=2) +
  ...
  annotate("text", x = "wetland", y = 20, label = "BCDFG") +
  annotate("text", x = "S1", y = 20, label = "D") 



p.co2 <- ggplot(df,aes(x=reorder(New.Name,Elevation_m,median,decreasing = TRUE),y=CO2_umol.L)) +
   geom_point(size=2) +
  geom_boxplot(fill="grey"#outlier.colour="red", outlier.shape=8,outlier.size=2
     ) +
  stat_summary(geom = "point", fun = "mean", col = "black", size = 3, shape = 24, fill = "red") +
...
theme_bw(base_size = 16) +
  ...
  annotate("text", x = "wetland", y = 900, label = "AC") 
