...
#CO2 AICc 
#AICc: (Intercept), scale(Watertemp_c), scale(log(WS_area_minus_pond)), scale(percent_DTW), scale(Elevation_m)
...
#CO2 BIC
#(Intercept), scale(Watertemp_c), scale(log(WS_area_minus_pond)), scale(percent_DTW), scale(Elevation_m)
...
Estimate = c(NA,0.5365, .7025,NA,-1.6719,#ch4 AIC
             NA,0.5365,.7025,NA,-1.6719,#ch4 BIC
             0.12097,-0.17423,.28578,0.16991,-1.12682, #CO2 AIC
             NA,-0.15097,0.29598,0.16235,-1.18767),#CO2 BIC
...
my_labeller <- as_labeller(c(CH4="CH[4]", CO2="CO[2]", AICc="AICc", BIC="BIC"), default = label_parsed)
...
p2 <- ggplot(k_df) + geom_point(
  aes(x=surface_area_m2,y=K600,fill=log10(pCO2_ppm)), shape=21,size=3)  + geom_text(aes(x=surface_area_m2,y=K600,label=BelowSat),vjust=-0.1,hjust=-.5,color="red",size=7) +
  ...
  scale_fill_gradientn(name= expression(paste("CO"[2] ," (",mu,"mol ", L^-1,")")), colours = terrain.colors(10),
                       breaks=c(log10(10), log10(30), log10(100),log10(300)), labels=c("10","30","100","300"))
