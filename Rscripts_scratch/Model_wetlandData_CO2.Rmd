...
grid_df$Var5[which(grid_df$Var5 == 1)] <- "SA_to_Vol_ratio"
grid_df$Var6[which(grid_df$Var6 == 1)] <- "surface_area_m2"
...
#let's remove Wetland 12 for this analysis
...
for (row in 2:nrow(grid_df)){
  ...
  if(length(f_2)==1){
    ...
    model <- lmer(
      as.formula(paste0("log(CO2_umol.L) ~ ","scale(",f_2[1],") +scale(",f_2[2],") + scale(",f_2[3],") + scale(",f_2[4],") + scale(",f_2[5],") + scale(",f_2[6],") + scale(",f_2[7],") + scale(",f_2[8],") + (1 |Wetland)")),data=df_1,REML=FALSE)
    summary <- summary(model)
