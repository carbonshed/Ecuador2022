...
df_CO2 <- read.csv(here::here("ProcessedData/All_CO2_samples_May29.csv"))

C02_middle <- df_CO2%>%filter(Location=="middle")%>%select(Site,Location,Date,pCO2_w_uatm)

C02_edge <- df_CO2 %>%group_by(Site,Date,Location)%>%filter(Location!="middle")%>%
dplyr::summarise(
  pCO2_w_uatm = mean(pCO2_w_uatm,rm.na=TRUE))

CO2_summary <- rbind(C02_middle,C02_edge)


ggplot(CO2_summary, aes(x=as.factor(Site), y=log(pCO2_w_uatm),color=Location)) + 
  geom_point()
...
dplyr::summarize(
  mean_uatm = mean(pCH4_w_uatm),
  sd_uatm = sd(pCH4_w_uatm),
  n_uatm = n())

CH4_table <- CH4_df %>%group_by(Site,Date)%>%
dplyr::summarize(
  mean_umol.L = mean(CH4_umol.L),
  sd_umol.L = sd(CH4_umol.L),
  n_uatm = n())

#write.csv(CH4_table,"~/OneDrive - University of North Carolina at Chapel Hill/Dissertation papers/Wetland GHG/ch4_summarytable_uatm.csv")

#write.csv(CH4_table,here::here("Kriddie/ch4_summarytable_uatm.csv"))


CO2_table <- CO2_summary%>%drop_na(pCO2_w_uatm) %>%group_by(Site,Date)%>%
dplyr::summarise(
  mean_uatm = mean(pCO2_w_uatm),
  sd_uatm = sd(pCO2_w_uatm),
  n_uatm = n())

#write.csv(CO2_table,"~/OneDrive - University of North Carolina at Chapel Hill/Dissertation papers/Wetland GHG/co2_summarytable_uatm.csv")
...
# Check normality of residuals
result <- aov(log(pCO2_w_uatm) ~ Location , data = CO2_df2 %>%filter(Site=="1"))
res <- residuals(result)
qqnorm(res) 
qqline(res)
shapiro.test(res) #normality
...
GHGsummary_df <-df %>%group_by(New.Name,Site,pCH4_w_uatm,pCO2_w_uatm,Elevation_m,percent_DTW,WS_area_minus_pond,waterTemp_c_yearly)%>%
  summarise(mean_ch4_uatm = mean(pCH4_w_uatm),
            mean_co2_uatm = mean(pCO2_w_uatm),
...
options(scipen = 100)
#methane
CH4_model <- aov(log1p(pCH4_w_uatm) ~ New.Name , data = df)
summary(CH4_model)
...
qqline(res)
shapiro.test(res) #normality
bartlett.test(log(pCH4_w_uatm)  ~ Site, data = df) 

#non-parametric
kruskal.test(log(pCH4_w_uatm) ~ Site , data = df)
#this on is good
conver_results_CH4<- kwAllPairsConoverTest(log(pCH4_w_uatm) ~ as.factor(New.Name), data = df, p.adjust="bonferroni")
...
CO2_model <- aov(log(pCO2_w_uatm) ~ New.Name , data = df)
summary(CO2_model)
# Check assumptions
...
qqline(res)
shapiro.test(res) #normality
bartlett.test(log(pCO2_w_uatm)  ~ Site, data = df) 

df <- df[order(df$New.Name),]
conver_results_CO2<- kwAllPairsConoverTest(log(pCO2_w_uatm) ~ as.factor(New.Name), data = df, p.adjust="bonferroni")
...
ggplot(df,aes(x=as.factor(Site),y=log(pCH4_w_uatm),color=Site)) + geom_point(size=3) 
ggplot(df,aes(x=as.factor(Site),y=log(pCO2_w_uatm),color=Site)) + geom_point(size=3) 

elev_df <- df%>%select(Site,Elevation_m)


#now you can do the figure
p.ch4 <- ggplot(df,aes(x=reorder(New.Name,Elevation_m,median,decreasing = TRUE),y=pCH4_w_uatm)) +
   geom_point(size=2) +
...
p.co2 <- ggplot(df,aes(x=reorder(New.Name,Elevation_m,median,decreasing = TRUE),y=pCO2_w_uatm)) +
   geom_point(size=2) +
  stat_summary(geom = "point", fun = "mean", col = "black", size = 3, shape = 24, fill = "red") +
  scale_y_log10() +
...
p.all <- ggarrange(p.ch4, p.co2, labels = c("a", "b"),heights=c(1.5,2), ncol = 1, nrow = 2)
...
gpmc_co2_mod1 <- lmer(log(mean_pCO2_uatm) ~ 
                    ...
                    scale(Elevation_m)+
                    (1 |Site), data =gmc)
gpmc_co2_mod2 <- lmer(log1p(mean_pCO2_uatm) ~ 
                    #Level 1 (time) 
                    Watertemp_c+ #GROUP mean centered!
                    #Level 2 (pond-level)
                    scale(gpm_Watertemp_c, scale=F)+ #GROUP means
...
gpmc_ch4_mod1 <- lmer(log1p(mean_pCH4_uatm) ~ 
                    #Level 1 (time) 
                    Watertemp_c+ #GROUP mean centered!
                    #Level 2 (pond-level)
                    scale(gpm_Watertemp_c, scale=F)+ #GROUP means
                    scale(Elevation_m)+
                    (1 |Site), data =gmc)
gpmc_ch4_mod2 <- lmer(log(mean_pCH4_uatm) ~ 
                    #Level 1 (time) 
                    Watertemp_c+ #GROUP mean centered!
                    #Level 2 (pond-level)
                    scale(gpm_Watertemp_c, scale=F)+ #GROUP means
...
p.co2 <- ggplot(df,aes(x=Watertemp_c,y=pCO2_w_uatm,color = reorder(New.Name,Elevation_m,median,decreasing = TRUE))) +
  geom_point(size=2)+
  geom_point(data=df,aes(x=Watertemp_c,y=pCO2_w_uatm),shape=1,size=2,color="black")+
  geom_smooth(method='lm',formula = "y ~ x",se=FALSE) +
  geom_point(data=gmc,aes(x=gpm_Watertemp_c,y=mean_pCO2_uatm,fill=Elevation_m),color="black",shape=23,size=4)+
    geom_point(data=gmc,aes(x=gpm_Watertemp_c,mean_pCO2_uatm),shape=3,size=1,color="black") +
  ylab(expression(italic(p)*paste("CO"[2] ," (",mu,"atm)"))) + xlab(xlab) +
  scale_color_viridis(name="Site",discrete = TRUE,guide = "none") +
  scale_y_continuous(transform="log",breaks=c(300,1000,3000,10000)) + 
    theme_bw(base_size = 16)

p.co2.ele <- ggplot() +
    geom_smooth(data=df,aes(x=Elevation_m,y=pCO2_w_uatm), method='lm',formula = "y ~ x",se=FALSE,color="black") +
  geom_point(data=df,aes(x=Elevation_m,y=pCO2_w_uatm,color=reorder(New.Name,Elevation_m,median,decreasing = TRUE)),size=2) +
  geom_point(data=df,aes(x=Elevation_m,y=pCO2_w_uatm),shape=1,size=2) +
  ylab("") +
...
p.ch4 <- ggplot() + 
  geom_smooth(data=gmc,aes(x=gpm_Watertemp_c,y=mean_pCH4_uatm),method='lm',formula = "y ~ x",se=FALSE,color="black") +
  geom_point(data=df,aes(x=Watertemp_c,y=pCH4_w_uatm,color=reorder(New.Name,Elevation_m,median,decreasing = TRUE)),size=2) +
  geom_point(data=df,aes(x=Watertemp_c,y=pCH4_w_uatm),color="black",shape=1, size=2) +
  geom_point(data=gmc,aes(x=gpm_Watertemp_c,y=mean_pCH4_uatm,fill=Elevation_m),shape=23,size=4)+
  geom_point(data=gmc,aes(x=gpm_Watertemp_c,y=mean_pCH4_uatm),color="black",shape=3,size=1)+
ylab(expression(italic(p)*paste("CH"[4] ," (",mu,"atm)"))) +
  xlab(xlab) +
  scale_color_viridis(name="Site",discrete = TRUE) +
  scale_y_continuous(transform="log10",breaks = c(3,10,30,100,300,1000,3000)) + 
  theme_bw(base_size = 16) +
...
p.ch4.ele <- ggplot() +
    geom_smooth(data=df,aes(x=Elevation_m,y=pCH4_w_uatm), method='lm',formula = "y ~ x",se=FALSE,color="black") +
  geom_point(data=df,aes(x=Elevation_m,y=pCH4_w_uatm,color=reorder(New.Name,Elevation_m,median,decreasing = TRUE)),size=2) +
  geom_point(data=df,aes(x=Elevation_m,y=pCH4_w_uatm),shape=1,size=2) +
  ylab("") +
 #   ylab(expression(italic(p)*paste("CH"[4] ," (",mu,"atm)"))) +
  xlab('Elevation (m)') +
  scale_color_viridis(name="Site",discrete = TRUE) +
...
library(cowplot)
full_1 <- plot_grid(p.ch4 + rremove("x.title") + rremove("legend") , p.co2+ rremove("legend"), ncol = 1, align = "v",labels=c("a","b"))


full_2 <- plot_grid(p.ch4.ele + rremove("x.title") +
#rremove("y.title") + 
  rremove("legend") ,p.co2.ele+ #rremove("y.title") +
  rremove("legend"),nrow=2,labels=c("c","d"))

full_3 <- ggarrange(full_1,full_2,heights=c(1,1.5), nrow=1)
  

full_4 <- ggarrange(leg,nrow=1)

full_5 <- ggarrange(full_3,NULL,full_4,heights = c(4,.02,1), nrow = 3)
...
k_df <- read.csv(here::here("ProcessedData/PondPaper_k600_May29.csv"))
...
p1 <- ggplot(k_df_summer%>%drop_na(Flux_umol_m2_s),aes(x=pCO2_w_uatm,y=Flux_umol_m2_s*10^-6*60*60*24)) + geom_point(aes(fill=New.Name),shape=21,size=3) +  
   geom_smooth(method='lm', color='black') + guides(fill=guide_legend(title="Site name"))+
  annotate("text",x=3500, y=.001, label=expression(paste(r^2 ," = 0.77")),size=5) +
...
p2 <- ggplot(k_df_summer %>%drop_na(Flux_umol_m2_s),aes(x=k_m_d,y=Flux_umol_m2_s*10^-6*60*60*24)) + geom_point(aes(fill=New.Name),shape=21,size=3) +  
   geom_smooth(method='lm', color='black') + guides(fill=guide_legend(title="Site name"))+
  annotate("text",x=1, y=.001, label=expression(paste(r^2 ," = 0.77")),size=5) +
      ylab(expression(italic(p)~paste("CO"[2] ,"  Evasion (mol ", m^-2," ", d^-1,")"))) +
#  xlab(expression(italic(k)*paste(" m/d"))) +
  scale_x_continuous(transform = "log10") + scale_y_continuous(transform = "log10") +
  theme_bw(base_size = 14) 
...
k_df <- read.csv(here::here("ProcessedData/PondPaper_k600_May29.csv"))
k_df_summer <- k_df%>%filter(Date<"2022-10-01")
k_df_summer_positvevalues <- k_df%>%filter(Date<"2022-10-01")%>%filter(k_m_d>0)
median(k_df$K600,na.rm = TRUE)
mean(k_df$K600,na.rm = TRUE)

median(k_df_summer$K600)
mean(k_df_summer$K600)

median(k_df_summer_positvevalues$K600)
mean(k_df_summer_positvevalues$K600)
median(k_df_summer_positvevalues$k_m_d)
mean(k_df_summer_positvevalues$k_m_d)
...
p2.1 <- ggplot(k_df_summer) + geom_point(
...
df_merge <- read.csv(here::here("ProcessedData/PondPaper_k600_May29.csv"))%>%rename("Site_Name"="New.Name")
...
gavi_df <- read.csv(here::here("ProcessedData/PondPaper_k600_May29.csv"))%>%filter(Site=="Wetland12")%>%select(Site, Date,AirPress_kpa,Watertemp_c,pCO2_ppm, CH4_umol.L,CO2_umol.L,pCO2_w_uatm,pCH4_w_uatm)
...
drone_GHG <- read.csv(here::here("ProcessedData/Drone_Gavi_sampling_GHG_june6.csv"))%>%select(Lat,Lon,Site, Site2, Date, AirPress_kpa,Watertemp_c,pCH4_w_uatm,pCO2_w_uatm,pCO2_ppm,CH4_umol.L, CO2_umol.L)

###

##pecent diff of dups
drone_GHG_summ2 <- drone_GHG%>%group_by(Site,Date,Site2)%>%summarise(
  CH4_min = min(pCH4_w_uatm),
  CH4_max = max(pCH4_w_uatm))
...
drone_GHG_summ <- drone_GHG%>%group_by(Lat,Lon,Site,Date,Site2)%>%summarise(
  pCH4_w_uatm = mean(pCH4_w_uatm,rm.na=TRUE),
  pCO2_w_uatm = mean(pCO2_w_uatm,rm.na=TRUE),
...
#summerize
df_summ <- df%>%group_by(Site)%>%drop_na(pCH4_w_uatm)%>%summarise(
  pCH4_uatm_ave = mean(pCH4_w_uatm,rm.na=TRUE),
  pCO2_uatm_ave = mean(pCO2_w_uatm,rm.na=TRUE))

df_melt <- df %>%select(Location,Date,Site2,pCH4_w_uatm,pCO2_w_uatm)%>%
    pivot_longer(
      cols = -c(Location,Date,Site2),  # Columns to be melted
      names_to = "gas",      # Name of the new key column
      values_to = "uatm"         # Name of the new value column
    )
 

p2 <- ggplot(df_melt,aes(x=gas, y=uatm,fill=Location)) +
  geom_boxplot() +
...
p3 <- ggplot(df,aes(x=pCO2_w_uatm, y=pCH4_w_uatm,color=Site)) + geom_point()  #+
