...
M_AIC_1 <- lmer(log(CO2_umol.L) ~ 
                  scale(BaroTemp_c_yearly)+ 
                  scale(waterTemp_c_yearly) +
                  scale(log(SurfaceArea_log)) +
                  scale(SA_to_Vol_ratio)
                  (1 |Wetland), data =df_1,REML = FALSE)
summary(M_AIC_1)
modelPerformance(M_AIC_1)

sum_M_CO2_AIC_W12 <- summary(M_CO2_AIC_W12)
coeff_M_CO2_AIC_W12 <- as.data.frame(sum_M_CO2_AIC_W12$coefficients)
coeff_M_CO2_AIC_W12$effects <- rownames(coeff_M_CO2_AIC_W12)

M_BIC_1 <- lmer(log(CO2_umol.L) ~ 
                  scale(BaroTemp_c_yearly)+ 
                 scale(waterTemp_c_yearly) +
                  (1 |Site), data =df_1,REML = FALSE)
summary(M_BIC_1)
modelPerformance(M_BIC_1)

#Without Wetland 12


#AIC: scale(BaroTemp_c_yearly), scale(waterTemp_c_yearly)
...
pred.mm <- ggpredict(M_BIC_2, terms = c("BaroTemp_c_yearly"#, "waterTemp_c_yearly", "SurfaceArea_log")
