...
M1 <- lmer(log(CO2_umol.L) ~ 
           ...
           #  scale(BaroTemp_c_day_eleAdjust) +
             scale(Elevation_m) +
...
##BIC
#Intercept), scale(percent_DTW), scale(Elevation_m)

M_BIC <- lmer(log(CO2_umol.L) ~ 
                scale(percent_DTW) +
                scale(Elevation_m) +
                  (1 |Site), data =df_1,REML = FALSE)
summary(M_BIC)
