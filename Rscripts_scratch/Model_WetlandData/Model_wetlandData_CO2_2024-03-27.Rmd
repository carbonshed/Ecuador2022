...
M2 <- lmer(log(CO2_umol.L) ~ 
                ...
                #  scale(Watershed_m2) +
                  scale(precip_mm_ave2) + 
                  (1 |Site), data =df%>%filter(Site!= "Wetland12"))
...
grid_df$Var3[which(grid_df$Var3 == 1)] <- "Elevation_m"
grid_df$Var4[which(grid_df$Var4 == 1)] <- "log(SA_to_Vol_ratio)"
grid_df$Var5[which(grid_df$Var5 == 1)] <- "AirTemp_c"
grid_df$Var6[which(grid_df$Var6 == 1)] <- "precip_mm_ave2"
#grid_df$Var7[which(grid_df$Var7 == 1)] <- "log(surface_area_m2)"
...
#AIC: (Intercept), scale(Watertemp_c), scale(waterTemp_c_yearly), scale(Elevation_m), scale(AirTemp_c)
    #This is where surface area is removed 
...
#BIC: scale(Watertemp_c), scale(waterTemp_c_yearly), scale(Elevation_m)
