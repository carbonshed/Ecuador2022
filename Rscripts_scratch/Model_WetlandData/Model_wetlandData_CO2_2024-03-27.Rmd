...
> about model selection including AIC and BIC
...
#df_saved <- read.csv(here::here("Wetlands/Wetland_df_MERGE_2024-03-27.csv"))
...
#remove Wetland 12?
df <- df_saved%>%filter(Site!="Wetland12")
...
hist(log(df$SA_to_Vol_ratio))
ggplot(df, aes(log(SA_to_Vol_ratio), num)) + geom_point()
...
boxplot(log(df$WS_area_minus_pond))
hist(log(df$WS_area_minus_pond))
ggplot(df, aes(log(WS_area_minus_pond), num)) + geom_point()

boxplot(log(df$percentpond))
hist(log(df$percentpond))
ggplot(df, aes(log(percentpond), num)) + geom_point()

boxplot(df$percent_DTW)
hist(df$percent_DTW)
ggplot(df, aes(percent_DTW, num)) + geom_point()
```

#Finding from step 1.
  > there are outliers but I checked, and these measurments are legit (I did change a few AirTemp_c measurments)
  > I transformed a few things - the response variable, CO2, SA_to_vol_ratio and surface area.
...
##findings:
  > remove Watertemp_day because it is correlates strongly with WaterTemp_c -- I think this might be more useful for future studies if you don't have a looger installed, you can just take a point measurments of water temp
  > instead of baro yearly data wich coorelate strongly with elevation (since that's how it's calculated), I think I'll straight up use elevation. Same idea with BaroTemp_c_day
  > I don't have a good theoretical reaseon to include solar radiation, so delete that too
...
#findings from step 5.2
  > Elevation and surface area are more coorelated than I'd realized. I suppose if I use the rule of anything under 10 is ok, than I am good, but it I want to use the more conservative <3, then I need to get rid of one or the other. Poop!!
  If I included wetland 12, this would not be a problem... AUG!
  > lets include elevation and SA to vol ratio and oit surface area for now...
...
> model 2 (no SA_to_Vol_ratio) is better than model 1 --- > but I think now that I have omitted wetland 12 AND log rransformed, it is better
...
> so this is interesting --- we have a bit of an issue in wich surface area and elevation are coorelated. But maybe this helps support keeping elevation and excluding surface area: When elevation AND SA are included, elevation is significant, SA is not. When Elevation is included and SA is excluded, r2 = .80. When SA is included and Elevation omited, R2 = .40
...
#df_1 <-df
df_1 <-df_saved%>%filter(Site!="Wetland12")
...
grid_df$Var3[which(grid_df$Var3 == 1)] <- "log(SA_to_Vol_ratio)"
grid_df$Var4[which(grid_df$Var4 == 1)] <- "log(WS_area_minus_pond)"
grid_df$Var5[which(grid_df$Var5 == 1)] <- "percent_DTW"
grid_df$Var6[which(grid_df$Var6 == 1)] <- "precip_mm_ave2"

grid_df$Var7[which(grid_df$Var7 == 1)] <- "Elevation_m"
grid_df$Var8[which(grid_df$Var8 == 1)] <- "log(surface_area_m2)"
...
for (row in 2:nrow(grid_df)){
  ...
  if(length(f_2)==1){
    model <- lmer(
    as.formula(paste0("log(CO2_umol.L) ~ ","scale(",f_2[1],") + (1 |Site)")),
    ...
    model <- lmer(
      as.formula(paste0("log(CO2_umol.L) ~ ","scale(",f_2[1],") +scale(",f_2[2],") + (1 |Site)")),data=df_1,REML=FALSE)
    summary <- summary(model)
    ...
    model <- lmer(
      as.formula(paste0("log(CO2_umol.L) ~ ","scale(",f_2[1],") +scale(",f_2[2],") + scale(",f_2[3],") + (1 |Site)")),data=df_1,REML=FALSE)
    summary <- summary(model)
    ...
    model <- lmer(
      as.formula(paste0("log(CO2_umol.L) ~ ","scale(",f_2[1],") +scale(",f_2[2],") + scale(",f_2[3],") + scale(",f_2[4],") + (1 |Site)")),data=df_1,REML=FALSE)
    summary <- summary(model)
    ...
    model <- lmer(
      as.formula(paste0("log(CO2_umol.L) ~ ","scale(",f_2[1],") +scale(",f_2[2],") + scale(",f_2[3],") + scale(",f_2[4],") + scale(",f_2[5],") + (1 |Site)")),data=df_1,REML=FALSE)
    summary <- summary(model)
    ...
    model <- lmer(
      as.formula(paste0("log(CO2_umol.L) ~ ","scale(",f_2[1],") +scale(",f_2[2],") + scale(",f_2[3],") + scale(",f_2[4],") + scale(",f_2[5],") + scale(",f_2[6],") + (1 |Site)")),data=df_1,REML=FALSE)
    summary <- summary(model)
    ...
    model <- lmer(
      as.formula(paste0("log(CO2_umol.L) ~ ","scale(",f_2[1],") +scale(",f_2[2],") + scale(",f_2[3],") + scale(",f_2[4],") + scale(",f_2[5],") + scale(",f_2[6],") + scale(",f_2[7],") + (1 |Site)")),data=df_1,REML=FALSE)
    summary <- summary(model)
    ...
    model <- lmer(
      as.formula(paste0("log(CO2_umol.L) ~ ","scale(",f_2[1],") +scale(",f_2[2],") + scale(",f_2[3],") + scale(",f_2[4],") + scale(",f_2[5],") + scale(",f_2[6],") + scale(",f_2[7],") + scale(",f_2[8],") + (1 |Site)")),data=df_1,REML=FALSE)
    summary <- summary(model)
    
    Dataframe[nrow(Dataframe) + 1,] <- list(toString(rownames(summary$coefficients)), summary$AIC[1], summary$AIC[2])
       
      } else if(length(f_2)==9) { 
    model <- lmer(
      as.formula(paste0("log(CO2_umol.L) ~ ","scale(",f_2[1],") +scale(",f_2[2],") + scale(",f_2[3],") + scale(",f_2[4],") + scale(",f_2[5],") + scale(",f_2[6],") + scale(",f_2[7],") + scale(",f_2[8],") + (1 |Site)")),data=df_1,REML=FALSE)
    summary <- summary(model)
...
df_1 <- df_saved
df_1$SurfaceArea_log <- log(df_1$surface_area_m2)

#Without Wetland 12


#AIC: (Intercept), scale(Watertemp_c), scale(waterTemp_c_yearly), scale(Elevation_m), scale(log(surface_area_m2)), scale(log(WS_area_minus_pond)), scale(percent_DTW)
...
```
#visualize results
```{r  plot it, echo=FALSE}


library(ggeffects)  # install the package first if you haven't already, then load it

# Extract the prediction data frame
pred.mm <- ggpredict(M_BIC_2, terms = c("Watertemp_c", "waterTemp_c_yearly", "Elevation_m"
                                        )
...
(ggplot(pred.mm) + 
   ...
   geom_ribbon(aes(x = x, ymin = log(predicted) - std.error, ymax = log(predicted) + std.error), 
               fill = "lightgrey", alpha = 0.5) +  # error band
   geom_point(data = df_1,                      # adding the raw data (scaled values)
              aes(x = BaroTemp_c_yearly, y = log(CO2_umol.L), colour = Wetland)) + 
   labs(x = "BaroTemp_c_yearly", y = "log(CO2_umol.L)", 
        title = "CO2 model") + 
   theme_minimal()
)

ggpredict(M_BIC_2, terms = c("BaroTemp_c_yearly", "Wetland"), type = "re") %>% 
   plot() +
   labs(x = "terms", y = "CO2", title = "CO2 model") + 
   theme_minimal() +
  scale_fill_manual(values=c("#CC0000", "#006600", "#669999", "#00CCCC", 
                             "#660099", "#CC0066", "#FF9999", "#FF9900",
                             "#CC0000", "#006600", "#669999","#FF9900"))

library(sjPlot)

# Visualise random effects 
(re.effects <- plot_model(M_BIC, type = "re", show.values = TRUE))
...
fit <- lmer(log(CO2_umol.L) ~ BaroTemp_c_yearly +waterTemp_c_yearly+ SurfaceArea_log +(1|Wetland), data=df)
# facet by group,
dat <- ggpredict(fit, terms = c("BaroTemp_c_yearly", "waterTemp_c_yearly", "SurfaceArea_log" ))
plot(dat)
 
# don't use facets, b/w figure, w/o confidence bands
dat <- ggpredict(fit, terms = c("BaroTemp_c_yearly", "waterTemp_c_yearly", "precip_mm_ave2" ))
plot(dat, colors = "bw", show_ci = FALSE)

#2
fit <- lmer(log(CO2_umol.L) ~ BaroTemp_c_yearly +waterTemp_c_yearly + (1|Wetland), data=df_saved)
# facet by group,
dat <- ggpredict(fit, terms = c("BaroTemp_c_yearly", "waterTemp_c_yearly" ))
plot(dat)

ggplot(data=df_saved) +
  geom_point(aes(x=waterTemp_c_yearly ,y=log(CO2_umol.L),color=BaroTemp_c_yearly),size=3)

```

#visualize using sjPlot::plot_model(mod)
```{r  plot it 2, echo=FALSE}

sjPlot::plot_model(M_BIC_1)
sjPlot::plot_model(M_BIC_2)
...
sjPlot::plot_model(M_BIC_1, 
                   axis.labels=c("Air Temperature yearly average","Water Temperature yearly average"),
                   show.values=TRUE, show.p=TRUE,
                   title="Effect of temperature on CO2 concentration")

sjPlot::tab_model(M_BIC_1, 
                  show.re.var= TRUE, 
                  pred.labels =c("(Intercept)", "Air Temp yearly ave (c)", "Water Temp yearly ave (c)"),
                  dv.labels= "CO2")

# Use the effects package --> effect function. term= the fixed effect you want to get data on, mod= name of your model.

effects_co2 <- effects::effect(term= "BaroTemp_c_yearly", mod= M_BIC_1)
summary(effects_co2) #output of what the values are
# Save the effects values as a df:
x_urch <- as.data.frame(effects_co2)
...
urchin_plot <- ggplot() + 
  #2
geom_point(data=df_saved, aes(BaroTemp_c_yearly, log(CO2_umol.L))) + 
  #3
#  geom_point(data=x_urch, aes(x=BaroTemp_c_yearly, y=fit), color="blue") +
  #4
  geom_line(data=x_urch, aes(x=BaroTemp_c_yearly, y=fit), color="blue") +
  #5
  geom_ribbon(data= x_urch, aes(x=BaroTemp_c_yearly, ymin=lower, ymax=upper), alpha= 0.3, fill="blue") +
  #6
  labs(x="Average yearly air temperature", y="log(CO2 umol/L)")
...
df_added_variable_plots <- df_saved
M_BIC_1 <- lmer(log(CO2_umol.L) ~ scale(BaroTemp_c_yearly) + scale(waterTemp_c_yearly) + (1 |Wetland), data =df_added_variable_plots,REML = FALSE)
df_added_variable_plots$M_BIC1_residuals <- residuals(M_BIC_1) 
