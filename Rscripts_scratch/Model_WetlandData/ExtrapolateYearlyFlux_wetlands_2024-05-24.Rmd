...
#Matthews et al. 2003 Environ. Sci. Technol.
df_full$Sc_ch4 <- 1897.8 + -114.28*df_full$WaterTemp_c + 32902*(df_full$WaterTemp_c)^2 + .039061*(df_full$WaterTemp_c)^3
...
#CO2 BIC
M_BIC_CO2 <-lme4::lmer(log(CO2_umol.L) ~ 
                scale(percent_DTW) +
                scale(Elevation_m) +
                  (1 |Site), data =df_1,REML = FALSE)
sum_BIC_CO2 <- summary(M_BIC_CO2)
...
df_full$M_AICc_CH4 <- exp(
  ...
  log(df_full$SA_to_Vol_ratio) * coeff_df[3,"Estimate"] +
...
df_full$M_BIC_CH4 <- exp(
  df_full$intercept +
  log(df_full$SA_to_Vol_ratio) * coeff_df[2,"Estimate"] +
    df_full$Elevation_m * coeff_df[3,"Estimate"]
  )
...
df_full$M_BIC_CO2 <- exp(
  df_full$intercept +
      df_full$percent_DTW * coeff_df[2,"Estimate"] +
    df_full$Elevation_m * coeff_df[3,"Estimate"]
  )
