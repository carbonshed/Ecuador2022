...
CO2 and k increases with Air temperature (increased respiration) - (ave day, ave year)
    > AirTemp_c
    > BaroTemp_c_yearly

CO2 and k increases with Water temperature (increased respiration) - (ave day, ave year)
    > WaterTemp_c
    > WaterTemp_c_day
    > waterTemp_c_yearly
...
precipitation effects CO2 concentration because it dilutes the water (ave of sum day and previous day), but also increases turbulance, increases k
    > Precio_mm_ave2

Solar radiation because stimulates gpp (ave day)
    > Solarrad_Wm2_daymean
    
Wind because it increases turbulance
  > windspeed_m_s
...
#boxplots galore
boxplot(log(df$Flux_umol_m2_s)) ##transformed
hist(log(df$Flux_umol_m2_s))
ggplot(df, aes(log(Flux_umol_m2_s), num)) + geom_point()
...
#grid_df$Var9 <- grid$Var9

grid_df$Var1[which(grid_df$Var1 == 1)] <- "BaroTemp_c_yearly"
grid_df$Var2[which(grid_df$Var2 == 1)] <- "waterTemp_c_yearly"
grid_df$Var3[which(grid_df$Var3 == 1)] <- "Water_minus_air_Temp"
grid_df$Var4[which(grid_df$Var4 == 1)] <- "waterTemp_c_day"
grid_df$Var5[which(grid_df$Var5 == 1)] <- "windspeed_m_s"
grid_df$Var6[which(grid_df$Var6 == 1)] <- "log(surface_area_m2)"
grid_df$Var7[which(grid_df$Var7 == 1)] <- "precip_mm_ave2"
grid_df$Var8[which(grid_df$Var8 == 1)] <- "solarrad_Wm2_daymean"
#grid_df$Var9[which(grid_df$Var9 == 1)] <- "SA_to_Vol_ratio"
...
for (row in 2:nrow(grid_df)){
  ...
  if(length(f_2)==1){
    model <- lmer(
    as.formula(paste0("log(Flux_umol_m2_s) ~ ","scale(",f_2[1],") + (1 |Wetland)")),
    ...
    model <- lmer(
      as.formula(paste0("log(Flux_umol_m2_s) ~ ","scale(",f_2[1],") +scale(",f_2[2],") + (1 |Wetland)")),data=df_1,REML=FALSE)
    summary <- summary(model)
    ...
    model <- lmer(
      as.formula(paste0("log(Flux_umol_m2_s) ~ ","scale(",f_2[1],") +scale(",f_2[2],") + scale(",f_2[3],") + (1 |Wetland)")),data=df_1,REML=FALSE)
    summary <- summary(model)
    ...
    model <- lmer(
      as.formula(paste0("log(Flux_umol_m2_s) ~ ","scale(",f_2[1],") +scale(",f_2[2],") + scale(",f_2[3],") + scale(",f_2[4],") + (1 |Wetland)")),data=df_1,REML=FALSE)
    summary <- summary(model)
    ...
    model <- lmer(
      as.formula(paste0("log(Flux_umol_m2_s) ~ ","scale(",f_2[1],") +scale(",f_2[2],") + scale(",f_2[3],") + scale(",f_2[4],") + scale(",f_2[5],") + (1 |Wetland)")),data=df_1,REML=FALSE)
    summary <- summary(model)
    ...
    model <- lmer(
      as.formula(paste0("log(Flux_umol_m2_s) ~ ","scale(",f_2[1],") +scale(",f_2[2],") + scale(",f_2[3],") + scale(",f_2[4],") + scale(",f_2[5],") + scale(",f_2[6],") + (1 |Wetland)")),data=df_1,REML=FALSE)
    summary <- summary(model)
    ...
    model <- lmer(
      as.formula(paste0("log(Flux_umol_m2_s) ~ ","scale(",f_2[1],") +scale(",f_2[2],") + scale(",f_2[3],") + scale(",f_2[4],") + scale(",f_2[5],") + scale(",f_2[6],") + scale(",f_2[7],") + (1 |Wetland)")),data=df_1,REML=FALSE)
    summary <- summary(model)
    ...
    model <- lmer(
      as.formula(paste0("log(Flux_umol_m2_s) ~ ","scale(",f_2[1],") +scale(",f_2[2],") + scale(",f_2[3],") + scale(",f_2[4],") + scale(",f_2[5],") + scale(",f_2[6],") + scale(",f_2[7],") + scale(",f_2[8],") + (1 |Wetland)")),data=df_1,REML=FALSE)
    summary <- summary(model)
    ...
    Dataframe[nrow(Dataframe) + 1,] <- list(toString(rownames(summary$coefficients)), summary$AIC[1], summary$AIC[2])
        ...
        model <- lmer(
      as.formula(paste0("log(Flux_umol_m2_s) ~ ","scale(",f_2[1],") +scale(",f_2[2],") + scale(",f_2[3],") + scale(",f_2[4],") + scale(",f_2[5],") + scale(",f_2[6],") + scale(",f_2[7],") + scale(",f_2[8],") + scale(",f_2[9],") + (1 |Wetland)")),data=df_1,REML=FALSE)
    summary <- summary(model)
...
#with wetland 12	AIC
#(Intercept), scale(BaroTemp_c_yearly), scale(waterTemp_c_yearly), scale(Water_minus_air_Temp), scale(waterTemp_c_day), scale(SA_to_Vol_ratio), scale(log(surface_area_m2)), scale(precip_mm_ave2), scale(solarrad_Wm2_daymean) 
df_1 <- df#%>%filter(Wetland!="Wetland12")

#AIC
M_AIC1 <- lmer(log(Flux_umol_m2_s) ~ 
                  scale(BaroTemp_c_yearly)+ 
                  scale(waterTemp_c_yearly) +
                  scale(Water_minus_air_Temp) +
                  scale(waterTemp_c_day) + 
...
summary(M_AIC1)
modelPerformance(M_AIC1)

#with wetland 12	BIC	
#(Intercept), scale(BaroTemp_c_yearly), scale(waterTemp_c_yearly), scale(Water_minus_air_Temp), scale(waterTemp_c_day), scale(SA_to_Vol_ratio), scale(log(surface_area_m2)), scale(solarrad_Wm2_daymean)

#BIC1
M_BIC1 <- lmer(log(Flux_umol_m2_s) ~ 
                  scale(BaroTemp_c_yearly)+ 
                  scale(waterTemp_c_yearly) +
                  scale(Water_minus_air_Temp) +
                  scale(waterTemp_c_day) + 
                  ...
                  (1 |Wetland), data =df_1,REML = FALSE)
summary(M_BIC1)
modelPerformance(M_BIC1)

M_BIC2 <- lmer(log(Flux_umol_m2_s) ~ 
                  BaroTemp_c_yearly+ 
                  waterTemp_c_yearly +
                  Water_minus_air_Temp +
                  waterTemp_c_day + 
...
sjPlot::tab_model(M_BIC2, 
                  show.re.var= TRUE, 
                  pred.labels =c("(Intercept)", "Year ave. Air Temp (c)","Year ave. Water Temp (c)","Air - water Temp (c)","Day ave. Water Temp (c)","Surface Area to Volumn Ratio","log(Surface Area m^2)","Day ave. Solar Radiation Wm2"),
                  dv.labels= "CO2 Flux prediction model")

```

#in the next chunk, I want to use this model to extrapolate flux for all wetlands
