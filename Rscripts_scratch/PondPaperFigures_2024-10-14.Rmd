...
ggplot(CH4_summer_df,aes(x=as.factor(Location),y=log(pCH4_ppm))) + geom_point()
ggplot(CH4_fall_df,aes(x=as.factor(Location),y=log(pCH4_ppm))) + geom_point()
...
C02_edge <-  df_CO2%>%filter(Location!="middle") %>%group_by(Site,Date,Location)%>% 
   summarize(mean= if(n() == 1) ppm_Corrected else mean(ppm_Corrected), .groups = 'drop',
             sd= if(n() == 1) ppm_Corrected else sd(ppm_Corrected), .groups = 'drop')
...
qqline(res)
shapiro.test(res) #normality
bartlett.test(log(ppm_Corrected)  ~ Location, data = CO2_df2%>%filter(Site=="12"))  #monovariance
