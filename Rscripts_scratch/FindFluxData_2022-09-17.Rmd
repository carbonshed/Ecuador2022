...
plot_ly(data=FluxData%>%filter(Date==date)%>%
  filter(DateTime_adjusted > as.POSIXct(paste(date, time_START), format="%Y-%m-%d %H:%M:%S", tz = "UTC"))%>%
  filter(DateTime_adjusted < as.POSIXct(paste(date, time_END), format="%Y-%m-%d %H:%M:%S", tz = "UTC")),
  x=~DateTime_adjusted,y=~Flux)
