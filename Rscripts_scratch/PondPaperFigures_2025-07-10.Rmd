...
#About: This Rmarkdown contains all code needed to produce the figures
published in "Water Temperature and Catchment Characteristics Drive
Variance in Greenhouse Gas Emissions from Small Ponds in a
Peatland-Rich, High-Altitude Tropical Ecosystem"

site descriptors includ: \> ave SA \> ave depth \> ave vol \> cathment
size \> elevation \> percent change in SA \> percent change in vol \>
percent veg \> DOC/TDN
...
WL_df <- WL_df%>%dplyr::select(DateTime,New.Name,Baro_kpa,WLTemp_c,depth_ave_m,WaterLevel_m,surface_area_m2,Volumn_m3)%>%rename(WaterTemp_c=WLTemp_c)
WL_df$WaterTemp_c <- ifelse(WL_df$WaterTemp_c < 0, 0, WL_df$WaterTemp_c)
WL_df$depth_ave_cm <- WL_df$depth_ave_m *100

WL_df_sum <- WL_df%>%group_by(New.Name)%>%
  dplyr::summarise(
...
sum_table <- read.csv(here::here("ProcessedData/PondPaper_k600_May29.csv"))%>%dplyr::select(Site,New.Name,Elevation_m,WS_area_minus_pond,DOC_mg.L,TDN_mg.L)
...
#summary stats ##see CO2_models_revision for details on models run

```{r site variance, echo=FALSE}

CO2_summary <- read.csv(here::here("ProcessedData/CO2_summary_June24.csv"))

CO2_summary_nowetland <- CO2_summary%>%filter(New.Name!="Wetland")

CO2_summary_sum <- CO2_summary_nowetland%>%group_by(New.Name)%>%
  summarise(pCO2_w_uatm_mean = mean(pCO2_w_uatm,na.rm=TRUE),
            pCO2_w_uatm_sd = sd(pCO2_w_uatm,na.rm=TRUE))

mean(CO2_summary_sum$pCO2_w_uatm_mean)
sd(CO2_summary_sum$pCO2_w_uatm_mean)

mean(CO2_summary_sum$pCO2_w_uatm_sd)

CH4_summary <- read.csv(here::here("ProcessedData/CH4samples_summary_June24.csv"))

CH4_summary_sum <- CH4_summary%>%filter(New.Name!="Wetland")%>%group_by(New.Name)%>%
  summarise(pCH4_w_uatm_mean = mean(pCH4_w_uatm,na.rm=TRUE),
            pCH4_w_uatm_sd = sd(pCH4_w_uatm,na.rm=TRUE))

mean(CH4_summary_sum$pCH4_w_uatm_mean)
sd(CH4_summary_sum$pCH4_w_uatm_mean)

mean(CH4_summary_sum$pCH4_w_uatm_sd)

#wetland center 

CH4_wetland_sum <- CH4_summary%>%filter(New.Name=="Wetland")%>%group_by(New.Name)%>%
  summarise(pCH4_w_uatm_mean = mean(pCH4_w_uatm,na.rm=TRUE),
            pCH4_w_uatm_sd = sd(pCH4_w_uatm,na.rm=TRUE))

CH4_wetland_sum

CO2_wetland_sum <- CO2_summary%>%filter(New.Name=="Wetland")%>%group_by(New.Name)%>%
  summarise(pCO2_w_uatm_mean = mean(pCO2_w_uatm,na.rm=TRUE),
            pCO2_w_uatm_sd = sd(pCO2_w_uatm,na.rm=TRUE))

CO2_wetland_sum$pCO2_w_uatm_mean
CO2_wetland_sum$pCO2_w_uatm_sd

##wetland margins

drone_df <- read.csv(here::here("ProcessedData/Drone_Gavi_forImporttoArcPro_june30.csv"))%>%dplyr::select(pCH4_w_uatm,pCO2_w_uatm)

summary(drone_df)

sd(drone_df$pCO2_w_uatm)
sd(drone_df$pCH4_w_uatm)
...
##see ExtrapolateYearlyFlux_wetlands_2025-07-07.R for details of the
airpressure model model
...
WL_df_midday <- WL_df_midday%>%group_by(New.Name,Date)%>%summarise(
...
df <- left_join(df,WL_df_midday,by=c("New.Name","Date"))
...
#water temp v elevation at all sites R squared over time, maybe RME over
time?
...
WL_df <- read.csv(here::here("Wetlands/WaterLevel_FINAL/WL_Wetland_all_FINAL.csv"))%>%dplyr::select("DateTime","Station","WLTemp_c","BaroTemp_c")%>%rename(Site=Station,AirTemp=BaroTemp_c)
WL_df$DateTime <- as.POSIXct(WL_df$DateTime,format="%Y-%m-%d %H:%M:%S",tz="UTC")
...
*Shapiro-Wilk Test:* Check normality of residuals. if value is greater
than 0.05, the data is normal. If it is below 0.05, the data
significantly deviate from a normal distribution \### *Bartlett's test:*
tests the hypothesis that our samples have equal variances. If the Chi-
Square test is significant (p \< . 05), then we can reject the null
hypothesis and assume that variances are unequal.
...
Formula: *Zscore = (x-mean)/std. deviation*

```{r site variance, echo=FALSE}
CO2_summary <- read.csv(here::here("ProcessedData/CO2_summary_June24.csv"))
CO2_summary$pCO2_log <- CO2_summary$pCO2_w_uatm

CO2_summary$Zscore_alldata <- (CO2_summary$pCO2_log - mean(CO2_summary$pCO2_log,na.rm=TRUE))/sd(CO2_summary$pCO2_log,na.rm=TRUE)

CH4_summary <- read.csv(here::here("ProcessedData/CH4samples_summary_June24.csv"))
CH4_summary$pCH4_log <- CH4_summary$pCH4_w_uatm

CH4_summary$Zscore_alldata <- (CH4_summary$pCH4_log - mean(CH4_summary$pCH4_log,na.rm=TRUE))/sd(CH4_summary$pCH4_log,na.rm=TRUE)

CH4_summary <- CH4_summary%>%group_by(New.Name)%>%
  mutate(Zscore_bysite=(pCH4_log - mean(pCH4_log,na.rm=TRUE))/sd(pCH4_log,na.rm=TRUE))
...
##see CO2_models_revision for details on models run
...
#Drivers of CH4 ##see CH4_models_revision
...
#DOC! ##see CO2_model revise

```{r GHG drviers, echo=FALSE}

df  <- read.csv(here::here("ProcessedData/predictor_variables_June24.csv"))%>%dplyr::select(New.Name,Elevation_m,percent_DTW,DOC_mg.L,TDN_mg.L)
...
p3 <- ggplot(data=df) +
  geom_point(aes(x=DOC_mg.L,y=TDN_mg.L,color=reorder(New.Name,Elevation_m,median,decreasing = TRUE))) +
...
df_precip <- read.csv(here::here("WeatherStation_LaVirgen/M5025_Precipitacion_Dato_validado.csv"))%>%rename(DateTime=Fecha,Precip_mm=Valor)

df_precip$DateTime <- as.POSIXct(df_precip$DateTime, format="%Y-%m-%d %H:%M:%S",tz="UTC")

df_precip$Date <- as.Date(df_precip$DateTime)
df_precip$year_month <- format(df_precip$Date,"%Y-%m")

df_precip$Date <- as.Date(df_precip$DateTime)


Precip_day <- df_precip%>%group_by(Date)%>%
  summarise(precip_accu_mm= sum(Precip_mm))
Precip_day$year_month <- format(Precip_day$Date,"%Y-%m")
Precip_day$month <- format(Precip_day$Date,"%m")
Precip_day$year <- format(Precip_day$Date,"%Y")



precip_plot <- ggplot(Precip_day%>%filter(year!="2021")
       , aes(x=month, y=precip_accu_mm,fill=month)) + 
  geom_boxplot(outlier.size = 1) + facet_wrap(~as.factor(year)) +
  ylab("Precipitation accumulation (mm per day)") +
...
#over study period
# Barplot
ggplot(Precip_day%>%filter(Date>"2022-06-01"&Date<"2023-08-01"), aes(x=Date, y=precip_accu_mm)) + 
  geom_bar(stat = "identity")

ggplot(Precip_day%>%filter(Date>"2022-06-01"&Date<"2022-11-01"), aes(x=Date, y=precip_accu_mm)) + 
  geom_bar(stat = "identity")
...
p.F.co2 <- ggplot(modeled_GHG_df %>%filter(DateTime > as.POSIXct("2022-07-12 00:00:00",tz='UTC') & DateTime < as.POSIXct("2022-08-12 00:00:00",tz='UTC'))) + 
  geom_point(aes(x=DateTime,y=F_CO2_mol_m2_d*12
    ...
    facet_wrap(~factor(New.Name, levels=c('S1', 'S2', 'S3','S4','S5','S6','S7','S8','S10','S11','S12','Wetland')), scales = "free_y") +
...
modeled_GHG_df_sub <- modeled_GHG_df %>%filter(DateTime > as.POSIXct("2022-07-12 00:00:00",tz='UTC') & DateTime < as.POSIXct("2022-08-12 00:00:00",tz='UTC'))

#averages
modeled_GHG_df_summ <- modeled_GHG_df_sub%>%group_by(New.Name)%>%summarise(
  F_CO2_mg_m2_d_mean = mean(F_CO2_mol_m2_d*12*1000,na.rm=TRUE),
  F_CH4_mg_m2_d_mean = mean(F_CH4_mol_m2_d*12*1000,na.rm=TRUE),
  F_mg.d_CO2_mean = mean(F_mol.d_CO2*12*1000,na.rm=TRUE),
  F_mg.d_CH4_mean = mean(F_mol.d_CH4*12*1000,na.rm=TRUE)
)

mean(modeled_GHG_df_summ$F_CO2_mg_m2_d_mean,na.rm=TRUE)
sd(modeled_GHG_df_summ$F_CO2_mg_m2_d_mean,na.rm=TRUE)

mean(modeled_GHG_df_summ$F_CH4_mg_m2_d_mean,na.rm=TRUE)
sd(modeled_GHG_df_summ$F_CH4_mg_m2_d_mean,na.rm=TRUE)
...
modeled_GHG_sum_df <- modeled_GHG_df %>%
  dplyr::select(New.Name,DateTime,F_mol.d_CH4,F_mol.d_CO2)
...
modeled_GHG_sum_df_2 <- modeled_GHG_sum_df%>%group_by(New.Name,Date)%>%
  summarise(
...
GHG_df_filter_sum <- GHG_df_filter%>%group_by(New.Name)%>%summarise(
...
#plot
options(scipen = 10)
p.ch4 <- ggplot(data=GHG_df_filter_sum, aes(x=factor(New.Name, levels=c('S10', 'S6', 'S11','S12','S5','S4','S7','S8','S3','S2','Wetland','S1')), y=F_CH4_mol_4week*12)) +
...
options(scipen = 10)
p.co2<- ggplot(GHG_df_filter_sum) + aes(x = factor(New.Name, levels=c('S10', 'S6', 'S11','S12','S5','S4','S7','S8','S3','S2','Wetland','S1')), y = F_CO2_mol_4week*12) +
...
df_Holg_peat <- df_Holg%>%filter(Reference=="Hamilton et al. (1994)"|Reference=="Kankaala et al. (2013)"|Reference=="Laurion et al. (2010)"|Reference=="Shirokova et al. (2013)"|Reference=="Pelletier et al. (2014)")

 test <- df_Holg%>%filter(Reference=="This Study") 

#log co2 v ch4
p <- ggplot() +
  geom_point(data=df_Holg#%>%filter(Area_ha < .5)
             ,aes(x=CO2_umol.L,y=CH4_umol.L),color="grey",size=3) +
 geom_point(data=df_Holg%>%filter(Reference=="This Study"),aes(x=CO2_umol.L,y=CH4_umol.L,fill=New.Name),shape=21,size=3) +
...
#summary of studies included in Holgerson & Raymond synthesis and \< .5
ha Barber et al. (1988) \<-- NOT Bartlett et al. (1992) Methane
emissions from tundra environments in the Yukon‐Kuskokwim Delta, Alaska
Bastviken et al (2002) Measurement of methane oxidation in lakes: a
comparison of methods Bastviken et al. (2004) \<-- could be, not
mentioned Casper et al. (2000) \<-- NOT Fallon et al. (1980) Hamilton et
al. (1994) \<-- peatlands\
Holgerson (2015) \<-- NOT\
Jonsson et al. (2003) Kankaala et al. (2013) \<-- peatlands\
Kling et al. (1991) Kortelainen et al (2006) CO2 or Juutinen et al.
(2009) CH4" Laurion et al. (2010) \<-- peatlands or thermokarst?? Lennon
et al. (2006) Miyajima et al. (1997) Natchimuthu et al. (2014) Pelletier
et al. (2014) \<-- peatlands Repo et al. (2007) \<-- peatlands Riera et
al. (1999) \<-- half and half Schulz et al. (2001) Smith and Lewis
(1992) Striegl and Michmerhuizen (1998) Utsumi et al. (1998a) Shirokova
et al. (2013) \<-- peatlands

> most of our sites lower than in Hamilton et al. (1994), The studyarea
> was locatedin the high boreal and low subarctic wetland region of the
> HudsonBay Lowland with the baseof operations at Moosonee, Ontario
> TheHBL is comprised of approximately *85-90% wetland areas* [Pala and
> Boissonneau,1982 and the entire area is underlain with impermeable
> glacial moraine clays and silts. These are *peatland ponds*
...
> sylvestris) cover 55–80% while *peatlands cover , 20% of the
> catchment* areas of the lakes

> our sites, all lower than Holgerson (2015) research forest in Tolland
> and Windham Counties, Connecticut, USA. The ponds are located in mixed
> deciduous-coniferous forest with minimal human disturbance. The soil
> types are Charlton-Chatfield Complex (Blacksmith, E8, Quarry, Woodpile
> Ponds) and Hinckley gravelly sandy loam
...
> colonized by *Sphagnum spp. mosses*. These thermokarst ponds are
> formed in depressions (1–3-m deep) left after the ice has melted below
> mineral mounds.

> Pelletier et al. (2014) where in the middle of our sites *boreal
> peatland*

> Shirokova et al. 2013 ocupied a very similar range to our sites
> forest, forest-tundra, and tundra landscape that has been developed on
...
> ours sites are similar range to 3 points in Delveccia et al 2019 (co2
> only) Mexican Cut Nature Preserve in the Elk Mountains of central
> Colorado, a pristine, subalpine wilderness area
...
WL_df <- WL_df%>%filter(DateTime>as.POSIXct("2022-07-12 00:00:00",tz="UTC")&DateTime<as.POSIXct("2023-07-12 00:00:00",tz="UTC"))%>%mutate(New.Name = case_match(Site, 'Wetland01' ~ 'S1', 'Wetland02' ~ 'S2','Wetland03' ~ 'S3','Wetland04' ~ 'S4','Wetland05' ~ 'S8','Wetland06' ~ 'S5','Wetland07' ~ 'S7','Wetland08' ~ 'S12','Wetland09' ~ 'S10','Wetland10' ~ 'S11','Wetland11' ~ 'S6','Wetland12' ~ 'Wetland'))%>%filter(New.Name!="Wetland")

WL_df_maxmean <- WL_df%>%group_by(New.Name)%>%summarise(SA_max=max(surface_area_m2,na.rm =  TRUE),SA_mean=mean(surface_area_m2,na.rm =  TRUE))

WL_df_day<-WL_df%>%group_by(Date,New.Name)%>%
  summarise(SA_day_ave = mean(surface_area_m2,na.rm = TRUE)
           )

WL_df_day <- WL_df_day%>%group_by(New.Name)%>%mutate(
    mean_val = mean(SA_day_ave,na.rm=TRUE)
  )

WL_df_day <- full_join(WL_df_day,WL_df_maxmean,by="New.Name")

WL_df_day$yr_mo <- format(WL_df_day$Date,format="%Y-%m")

WL_df_day$percent_change <- abs((WL_df_day$SA_mean-WL_df_day$SA_day_ave)/WL_df_day$SA_mean * 100)

test <- WL_df_day%>%group_by(New.Name)%>%summarise(min(percent_change),mean(percent_change),max(percent_change))
...
facet_wrap(~factor(New.Name, levels=c('S1', 'S2', 'S3','S4','S5','S6','S7','S8','S10','S11','S12')), scales = "free_y", nrow = 4, strip.position = "top") +
